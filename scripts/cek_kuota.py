...
class KuotaChecker:
    """Kelas untuk mengecek kuota XL menggunakan API"""
    
    @classmethod
    async def cek_kuota(cls, msisdn):
        """
        ...
        try:
            ...
            # Buat URL lengkap
            api_url = f"{base_url.rstrip('/')}/cek-kuota"
            ...
            logger.debug(f"Cek kuota request: URL={api_url}, Headers={headers}, Data={data}")
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(api_url, headers=headers, json=data) as response:
                    ...
                    logger.debug(f"Cek kuota response: Status={status_code}, Data={response_data}")
                    ...
                    if status_code == 200 and response_data.get('success', False):
                        ...
                        return {
                            'success': False,
                            'error': response_data.get('message', 'Terjadi kesalahan saat cek kuota'),
                            'data': {}
                        }
